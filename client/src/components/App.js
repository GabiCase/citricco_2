...
class App extends Component {
  ...
  removeFromCart = (productToRemove) => {
    const cartUpdated = this.state.cart.filter((product) => product !== productToRemove)
  ...
  increase = (product) => {
    ...
    itemIncrease && itemIncrease.quantity >= 0 ? itemIncrease.quantity++ : itemIncrease.quantity = 0
  ...
  render() {
    ...
    return (
      <>
        <Navigation cart={this.state.cart} cartChanged={this.cartChanged} setTheUser={this.setTheUser} loggedInUser={this.state.loggedInUser} />
        <Switch>
          <Route path="/" exact render={() => <Index />} />
          <Route path="/products/all" render={() => <ProductsList addToCart={this.addToCart} setTheUser={this.setTheUser} loggedInUser={this.state.loggedInUser} />} />
          <Route
